...
class MyBaseProto(getattr(asyncio, 'Protocol', object)):
    connected = None
    done = None
...
class MyDatagramProto(getattr(asyncio, 'DatagramProtocol', object)):
    done = None
...
class MyReadPipeProto(getattr(asyncio, 'Protocol', object)):
    done = None
...
class MyWritePipeProto(getattr(asyncio, 'BaseProtocol', object)):
    done = None
...
class MySubprocessProtocol(getattr(asyncio, 'SubprocessProtocol', object)):
...
@unittest.skip('no AbstractEventLoop')
...
@unittest.skip('no event loop policy')
...
class TestPyGetEventLoop(GetEventLoopTestsMixin, unittest.TestCase):
    ...
    _get_running_loop_impl = events._get_running_loop
    _set_running_loop_impl = events._set_running_loop
    get_running_loop_impl = events.get_running_loop
    get_event_loop_impl = events.get_event_loop


@unittest.skip('no socket')
