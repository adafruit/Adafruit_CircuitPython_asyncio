...
class BaseTaskTests:
    ...
    @unittest.skip('no generic alias')
...
class PyIntrospectionTests(test_utils.TestCase, BaseTaskIntrospectionTests):
    _register_task = staticmethod(tasks._register_task)
    _unregister_task = staticmethod(tasks._unregister_task)
    _enter_task = staticmethod(tasks._enter_task)
    _leave_task = staticmethod(tasks._leave_task)
...
class PyCurrentLoopTests(BaseCurrentLoopTests, test_utils.TestCase):
    ...
    def new_task(self, coro):
        return tasks.Task(coro, loop=self.loop)
